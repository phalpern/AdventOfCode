...
def move(point, offset, grid):
    """Return `point` offset by `offset`, where both `point` and `offset` are numpy
    arrays of length 2. Returns `None` if move is not a valid move."""
    result = point + offset
    # Fail if out of bounds
    if (result < (0, 0)).any() or (grid.shape <= result).any(): return None
    # Fail if move would gain more than 1 in altitude.
    if getElem(grid, result) - getElem(grid, point) > 1: return None
    return result

def climbHill(start, target, grid):
    """Find the shortest path from `start` to `target` within `grid` and return the
    number of steps."""
    ...
    while workQueue:
        ...
        if (pos == target).all():  # Found a valid path
            maxDepth = depth       # Any path longer than this should be ignored
            continue
        for direction in ((0, -1), (0, 1), (-1, 0), (1, 0)):
            newpos = move(pos, direction, grid)
            if newpos is None: continue
            workQueue.append(((path, newpos), depth + 1))
    return maxDepth
...
steps = climbHill(start, target, grid)
