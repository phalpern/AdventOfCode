...
# Add the version of packet and subpackets
...
def recursivelyAddVersions(packet):
    sum = packet.version
    if type(packet.value) == list:
        for subpacket in packet.value:
            sum += recursivelyAddVersions(subpacket)
    return sum
...
for hexstr in infile:
    ...
    packet = Packet(bs)
    print(packet)
    print("versions =", recursivelyAddVersions(packet))
