...
class Board:
    ...
    def read(self, infile):
        ...
        self.squares = [ ]
        for row in range(5):
            for str in re.split(' +', infile.readline().strip()):
                self.squares.append(int(str))
        assert len(self.squares) == 25
...
lastwin = ''
lastcall = -1
for call in randomSeq:
    for board in boards:
        if not board.bingo and board.mark(call):
            lastwin = board
            lastcall = call


print("Final bingo on {}!".format(lastcall))
lastwin.print()
print("Score = {}".format(lastwin.score(lastcall)))
