...
int computeActivations(const std::vector<std::string>& grid, Beam initialBeam)
{
  const int nRows = grid.size();
  const int nCols = grid.front().size();

  std::vector activations( nRows, std::vector( nCols, NO_DIR ) );

  std::vector workQueue{ 1, initialBeam };
  ...
  DEBUG2(printActivations(activations));
...
int main(int argc, char *argv[])
{
  ...
  const int maxCol = grid.front().size() - 1;

  int result = 0;
  for (int row = 0; row <= maxRow; ++row)
  {
    result = std::max(result, computeActivations(grid, { row, 0, RIGHT }));
    result = std::max(result, computeActivations(grid, { row, maxCol, LEFT }));
  }

  for (int col = 0; col <= maxCol; ++col)
  {
    result = std::max(result, computeActivations(grid, { 0, col, DOWN }));
    result = std::max(result, computeActivations(grid, { maxRow, col, UP }));
  }
