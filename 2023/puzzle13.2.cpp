...
// Determine if a can be made equal to b by applying a single smudge.
// Return 0 if a == b, 1 if a single smudge would make them equal, or 2 if
// more than one smudge would be needed.
int findSmudge(const Slice& a, const Slice b)
{
  ...
  for (int i = 0; i < a.size(); ++i)
  {
    if (a[i] != b[i])
    {
      ++smudges;
      if (smudges > 1)
        return 2;  // Cannot require more than one smudge
    }
  }
...
int findReflection(int n, GetSlice&& gs)
{
  for (int refLine = 1; refLine < n; ++refLine)
  {
    ...
    for (int a = refLine - 1, b = refLine; ; --a, ++b)
    {
      ...
      {
        if (smudges == 0)
          break; // Too few smudges; refLine is not the line of reflection
        return refLine;
      }
      ...
      smudges += findSmudge(sa, sb);

      if (smudges > 1)
        break;  // Too many smudges; refLine is not the line of reflection
    }
  }
