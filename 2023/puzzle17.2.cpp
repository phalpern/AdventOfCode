...
class StepToHeatLoss
{
  ...
  int calcIdx(const Step& s) const
  {
    ...
    constexpr int nCounts = 10;
    // Since repeat count is typically in range [1-10], subtract one, but add
...
public:
  StepToHeatLoss(int nRows, int nCols)
    ...
    : m_array(nRows * nCols * 4 * 10 + 1, INT_MAX)
    , m_nRows(nRows)
    , m_nCols(nCols)
    { }
...
// Compute up to 3 possible steps to execute next.  Note that some of the
...
std::array<Step, 3> calcNextSteps(const Step& fromStep)
{
  ...
  for (auto& resultStep : result)
  {
    ...
    resultStep.repeatCount = 1;
    resultStep.row = fromStep.row + deltas[dir].first;
    resultStep.col = fromStep.col + deltas[dir].second;

    if (dir == fromStep.direction || 0 == fromStep.repeatCount)
      resultStep.repeatCount = fromStep.repeatCount + 1;
    else if (fromStep.repeatCount < 4)
      // Must continue in same direction for at least 4 blocks.  Invalidate a
